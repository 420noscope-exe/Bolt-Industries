...
const int repeatDelayInitial = 10000;
const int repeatDelayRepeat = 10000;
...
const char sKeysForRows[5][18] = { { '`',  '1',  '2',  '3',  '4',  '5',  '6',  '7',  '8',  '9',  '0',  '-',  '=', 0x00, 0xB2, 0xD1, 0xD2, 0xD3 },
                                  ...
                                  { KEY_CAPS_LOCK, 0x00,  'a',  's',  'd',  'f',  'g',  'h',  'j',  'k',  'l',  ';', '\'', 0xB0, 0x00, 0x00, 0x00, 0x00 },
...
void ledCB(bool numlock, bool capslock, bool scrolllock, bool compose, bool kana, void *cbData) {
  ...
  (void) kana;
  (void) cbData;
  digitalWrite(LED_1, capslock ? HIGH : LOW);
}
...
void setup() {
  ...
  //Callback for LEDs is implemented in arduino-pico by earlephilhower
    ...
    for ( int j = 0; j < 18; ++j ) {
      if ( sKeysForRows[ i ][ j ] != 0x00 ) {
        ...
        if ( columnPressed(Col_0 + j) ) {
          ...
          if ( !sKeyPressed[ i ][ j ] ) {
            // Initial keypress
            keyboardPress(sKeysForRows[ i ][ j ]);
